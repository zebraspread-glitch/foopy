...
const GAME_ABBR: Record<string, string> = {
  ...
  Essendon: "ESS", Fitzroy: "FTZ", Footscray: "FTZ", Fremantle: "FRE",
  Geelong: "GEE", "Gold Coast": "GC", "Greater Western Sydney": "GWS",
  Hawthorn: "HAW", Melbourne: "MEL", "North Melbourne": "NM",
  "Port Adelaide": "PA", Richmond: "RIC", "St Kilda": "STK",
  "South Melbourne": "SM", Sydney: "SYD", "West Coast": "WCE", "Western Bulldogs": "WB",
};

const GAME_COLORS: Record<string, string> = {
  Adelaide: "#002b5c", "Brisbane Lions": "#a50034", Carlton: "#031a35",
  Collingwood: "#111217", Essendon: "#cc0000", Fitzroy: "#003087",
  Footscray: "#2563eb", Fremantle: "#4b1979", Geelong: "#003b73",
  "Gold Coast": "#c8102e", "Greater Western Sydney": "#f47920", Hawthorn: "#4d2004",
...
type LeastShotRecord = {
  team: string; score: string; shots: number;
  opponent: string; opponentScore: string;
  round: string; year: number; venue?: string;
};

const LEAST_SHOT_RECORDS: LeastShotRecord[] = [
  { team: "St Kilda", score: "1.0 (6)",  shots: 1, opponent: "Collingwood", opponentScore: "65",  round: "R13", year: 1898, venue: "Victoria Park" },
  { team: "St Kilda", score: "1.0 (6)",  shots: 1, opponent: "Essendon",    opponentScore: "85",  round: "R9",  year: 1899, venue: "East Melbourne" },
  { team: "St Kilda", score: "0.1 (1)",  shots: 1, opponent: "Geelong",     opponentScore: "162", round: "R17", year: 1899, venue: "Corio Oval" },
  { team: "Fitzroy",  score: "0.1 (1)",  shots: 1, opponent: "Footscray",   opponentScore: "66",  round: "R5",  year: 1953, venue: "Western Oval" },
];
...
function LeastShotMatchBox({ record }: { record: LeastShotRecord }) {
  const parsed = parseScoreStr(record.score);
  const detail = [`${record.shots} scoring shot`, record.venue, `${record.round} ${record.year}`].filter(Boolean).join(" · ");
  return (
    <div style={cardStyle}>
...
export default function LeastShotsPage() {
  ...
  return (
    <main style={pageStyle} className="page-enter">
      <header style={headerStyle}>
        ...
        <span style={titleStyle}>Fewest Scoring Shots</span>
      </header>

      <div style={listStyle}>
        {LEAST_SHOT_RECORDS.map((r, i) => (
          <div key={i}>
            <span style={rankLabelStyle}>=1</span>
            <LeastShotMatchBox record={r} />
          </div>
        ))}
      </div>
    </main>
