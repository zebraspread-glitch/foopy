...
type ScoreRecord = {
  team: string; score: string; total: number;
...
function MatchBox({ record, rank }: { record: ScoreRecord; rank: number }) {
  const parsed = parseScoreStr(record.score);
  const detail = [record.venue, `${record.round} ${record.year}`].filter(Boolean).join(" · ");
  return (
    <div style={cardStyle}>
      <div style={{ display: "flex", flexDirection: "column" as const, gap: 2, padding: "10px 0 0" }}>
        ...
        <TeamBar team={record.opponent} total={record.opponentScore ?? null} faded />
  ...
  return (
    <main style={pageStyle} className="page-enter">
      <header style={headerStyle}>
        ...
        <span style={titleStyle}>Highest Scores</span>
      </header>

      <div style={listStyle}>
        {TOP_SCORES.map((r, i) => (
          <div key={i}>
            <span style={rankLabelStyle}>{i === 0 ? "=1" : `${i + 1}`}</span>
            <MatchBox record={r} rank={i + 1} />
          </div>
        ))}
      </div>
    </main>
