...
type BehindRecord = {
  team: string; score: string; behinds: number;
  opponent: string; opponentScore: string;
  round: string; year: number; venue?: string;
};

const BEHIND_RECORDS: BehindRecord[] = [
  { team: "Hawthorn", score: "25.41 (191)", behinds: 41, opponent: "St Kilda",       opponentScore: "103", round: "R6",  year: 1977, venue: "Princes Park" },
  { team: "Geelong",  score: "22.35 (167)", behinds: 35, opponent: "Melbourne",      opponentScore: "79",  round: "R16", year: 1981, venue: "M.C.G." },
  { team: "Melbourne",score: "12.34 (106)", behinds: 34, opponent: "North Melbourne",opponentScore: "49",  round: "R7",  year: 1940, venue: "Arden St" },
];
...
function BehindMatchBox({ record }: { record: BehindRecord }) {
  const parsed = parseScoreStr(record.score);
  const detail = [`${record.behinds} behinds`, record.venue, `${record.round} ${record.year}`].filter(Boolean).join(" · ");
  return (
    <div style={cardStyle}>
...
export default function MostBehindsPage() {
  ...
  return (
    <main style={pageStyle} className="page-enter">
      <header style={headerStyle}>
        ...
        <span style={titleStyle}>Most Behinds in a Game</span>
      </header>

      <div style={listStyle}>
        {BEHIND_RECORDS.map((r, i) => (
          <div key={i}>
            <span style={rankLabelStyle}>{i + 1}</span>
            <BehindMatchBox record={r} />
          </div>
        ))}
      </div>
    </main>
