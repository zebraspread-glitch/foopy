...
import { TEAM_PASS_COST } from "@/app/lib/passes";
...
export function TeamPassSection({ teamName, accentColor }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Check if current user already owns this team pass
    ...
    <section style={{
        ...
        >
          {loading ? (
            ...
            <>
              ...
              Get Pass · 🪙 {TEAM_PASS_COST.toLocaleString()}
            </>
          )}
        </button>
      ...
      {success && (
        <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: "#22c55e" }}>
          Pass activated! You'll earn rewards when {teamName} wins.
        </div>
      )}
    </section>
  );
